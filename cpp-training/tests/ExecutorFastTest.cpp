...
TEST(ExecutorFastTest, should_return_N_and_x_plus_2_given_status_is_fast_command_is_L_and_facing_is_E)
{
    std::unique_ptr<Executor> executor(Executor::NewExecutor({0, 0, 'E'}));
    executor->Execute("FL");
...
TEST(ExecutorFastTest, should_return_S_and_x_plus_2_given_status_is_fast_given_command_is_R_and_facing_is_E)
{
    std::unique_ptr<Executor> executor(Executor::NewExecutor({0, 0, 'E'}));
    executor->Execute("FR");
